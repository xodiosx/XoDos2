// workflow.dart - Complete with real BusyBox extraction progress
...
class Util {
  ...
  static Stream<ExtractionProgress> extractWithBusyBoxProgress({
    ...
    try {
      ...
      G.updateText.value = AppLocalizations.of(G.homePageStateContext)!.preparingExtraction;
      ...
      await for (final chunk in xzOutput) {
        ...
        yield ExtractionProgress(
          ...
          currentFile: currentFile.isNotEmpty ? currentFile : AppLocalizations.of(G.homePageStateContext)!.decompressing,
        );
        
        // Update UI text
        if (currentFile.isNotEmpty) {
          G.updateText.value = '${AppLocalizations.of(G.homePageStateContext)!.extractingFiles}: $currentFile (${percentage.toStringAsFixed(1)}%)';
        } else {
          G.updateText.value = '${AppLocalizations.of(G.homePageStateContext)!.decompressing} (${percentage.toStringAsFixed(1)}%)';
        }
      }
      ...
      yield ExtractionProgress(
        ...
        totalBytes: totalSize,
        percentage: 100.0,
        currentFile: AppLocalizations.of(G.homePageStateContext)!.extractionComplete,
      );
...
""");
  ...
  static Future<void> extractContainerWithProgress() async {
    ...
    try {
      ...
      G.updateText.value = AppLocalizations.of(G.homePageStateContext)!.fallbackExtraction;
