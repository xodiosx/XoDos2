...
class Util {
  ...
  static dynamic getGlobal(String key) {
    ...
    switch (key) {
      ...
      case "useX11" : return b ? G.prefs.getBool(key)! : (value){G.prefs.setBool(key, value); return value;}(true);
...
class Workflow {
  ...
  static bool _x11LaunchDone = false;   // <-- ADD THIS
...
""");
  ...
  // 2. Already launched in this session?
  if (_x11LaunchDone) {
    debugPrint(' X11 already launched – skipping duplicate');
    return;
  }

  // 3. Mark as done BEFORE the call (so even if an error occurs later,
  //    we won't call it again from other places)
  _x11LaunchDone = true;

  // 4. Launch via the untouched, original X11Flutter method
  await X11Flutter.launchXServer(
    "${G.dataPath}/usr/tmp",
    "${G.dataPath}/usr/share/X11/xkb",
    [":4"],
  );
}
...
export VK_ICD_FILENAMES=/data/user/0/com.xodos/files/usr/share/vulkan/icd.d/virtio_icd.aarch64.json
" > \$DATA_DIR/usr/opt/drv
...
// =========================================================================
// Local Archive Installer Dialog (with proot prompt) – localized version
// =========================================================================
...
class LocalArchiveInstaller {
  static Future<bool?> showDialog(BuildContext context) {
    return showGeneralDialog<bool?>(
      ...
      barrierLabel: AppLocalizations.of(context)!.installTitle,
...
class __LocalArchiveDialogState extends State<_LocalArchiveDialog> {
  ...
  Future<void> _checkForArchive() async {
    ...
    if (!dir.existsSync()) {
      setState(() {
        _statusMessage = loc.downloadFolderNotFound;
        _error = true;
      });
      return;
    }
    ...
    if (xzFile.existsSync()) {
      ...
      setState(() {
        ...
        _statusMessage = loc.fallbackNoArchive;
      });
      return;
    }
    setState(() {
      _statusMessage = loc.foundArchive(_archivePath.split('/').last);
    });
  }

  // --- size parsing (unchanged) ---
...
''';
    ...
    setState(() {
      _progress = 0.0;
      _statusMessage = loc.prootProgress('0');
    });
    final completer = Completer<int>();
    _extractPty!.output
        .cast<List<int>>()
      ...
      if (percent != null && mounted) {
        setState(() {
          _progress = (percent / 100.0).clamp(0.0, 1.0);
          _statusMessage = loc.prootProgress(percent.toStringAsFixed(0));
        });
      }
    });
    _extractPty!.exitCode.then((code) {
  ...
  Future<void> _startMainAction() async {
    ...
    if (_noArchiveFallback) {
      setState(() {
        _extracting = true;
        _statusMessage = loc.installingBasicShell;
      });
      ...
      if (ok) {
        setState(() {
          ...
          _extracting = false;
          _progress = 1.0;
          _statusMessage = loc.basicShellInstalled;
        });
      } else {
        setState(() {
          _error = true;
          _extracting = false;
          _statusMessage = loc.basicShellFailed;
        });
      }
      return;
    }
    ...
    if (!envOk) {
      setState(() {
        _error = true;
        _extracting = false;
        _statusMessage = loc.environmentSetupFailed;
      });
      return;
    }
    setState(() { _statusMessage = loc.calculatingSize; });
    ...
    if (totalSize <= 0) {
      setState(() {
        _error = true;
        _extracting = false;
        _statusMessage = loc.cannotDetermineSize;
      });
      return;
    }
    ...
    if (mainExit != 0) {
      setState(() {
        _error = true;
        _extracting = false;
        _statusMessage = loc.extractionFailed(mainExit.toString());
      });
      return;
    }
    // Check for proot.tar.xz
      ...
      if (installProot == true) {
        ...
        if (prootExit == 0) {
          setState(() {
            ...
            _extracting = false;
            _progress = 1.0;
            _statusMessage = loc.installationCompleteWithProot;
          });
        } else {
          setState(() {
            _error = true;
            _extracting = false;
            _statusMessage = loc.extractionFailed(prootExit.toString());
          });
        }
        return;
      }
    }
    // No proot or cancelled
    setState(() {
      _done = true;
      _extracting = false;
      _progress = 1.0;
      _statusMessage = loc.installationComplete;
    });
  }
  ...
  Widget build(BuildContext context) {
    ...
    return WillPopScope(
      ...
      child: AlertDialog(
        title: Text(loc.installTitle),
        content: Column(
          mainAxisSize: MainAxisSize.min,
          children: [
            if (_noArchiveFallback && !_extracting && !_done)
              Text(_statusMessage),
            if (!_archiveFound && !_noArchiveFallback && !_error)
              Text(loc.lookingForArchive(_downloadDir)),
            if (_error)
              Text(
                _statusMessage.isNotEmpty ? _statusMessage : loc.errorOccurred,
