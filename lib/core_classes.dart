...
// ============================================================================
// Environment Setup Class (Termux-like login logic)
// ============================================================================

class EnvironmentSetup {
  static final EnvironmentSetup _instance = EnvironmentSetup._internal();
  
  late String _basePath;
  late String _containerId;
  late Map<String, String> _envVars;
  
  factory EnvironmentSetup() {
    return _instance;
  }
  
  EnvironmentSetup._internal() {
    _envVars = {};
  }
  
  /// Initialize the environment setup with base paths
  void initialize({required String basePath, String containerId = "0"}) {
    _basePath = basePath;
    _containerId = containerId;
    _setupEnvironment();
  }
  
  /// Set up all environment variables and directories
  void _setupEnvironment() {
    // Define key paths
    final containerPath = '$_basePath/containers/$_containerId';
    final prefixPath = '$_basePath/usr';
    final homePath = '$_basePath/home';
    final tmpPath = '$prefixPath/tmp';
    final containerTmpPath = '$containerPath/tmp';
    
    // 1. Create symlink for tmp directory (like Termux does)
    _createSymlink(containerTmpPath, tmpPath);
    
    // 2. Set core environment variables
    _setEnv('PREFIX', prefixPath);
    _setEnv('HOME', homePath);
    _setEnv('TMPDIR', tmpPath);
    
    // 3. Setup PATH
    final systemPath = Platform.environment['PATH'] ?? '';
    _setEnv('PATH', '$prefixPath/bin:$systemPath:/system/bin');
    
    // 4. Setup LD_LIBRARY_PATH
    final systemLib = '/system/lib64';
    _setEnv('LD_LIBRARY_PATH', '$prefixPath/lib:$systemLib');
    
    // 5. Setup fontconfig
    _setEnv('FONTCONFIG_PATH', '$prefixPath/etc/fonts');
    _setEnv('FONTCONFIG_FILE', '$prefixPath/etc/fonts/fonts.conf');
    
    // 6. Create directories (like Termux's post-install)
    _createDirectory(tmpPath);
    _createDirectory(homePath);
    
    // 7. Additional LD_LIBRARY_PATH entries
    final currentLdPath = _envVars['LD_LIBRARY_PATH'] ?? '';
    _setEnv('LD_LIBRARY_PATH', '/lib64:$currentLdPath:$prefixPath/lib');
    
    // 8. Additional PATH entries
    final currentPath = _envVars['PATH'] ?? '';
    _setEnv('PATH', '$currentPath:$prefixPath/bin:/bin');
    
    // 9. X11 and graphics environment variables
    _setEnv('DISPLAY', ':4');
    _setEnv('XDG_RUNTIME_DIR', '$tmpPath/');
    _setEnv('X11_UNIX_PATH', '$tmpPath/.X11-unix');
    
    // 10. Vulkan ICD
    _setEnv('VK_ICD_FILENAMES', '$prefixPath/share/vulkan/icd.d/wrapper_icd.aarch64.json');
    
    // 11. TMPDIR and XDG directories
    _setEnv('TMPDIR', tmpPath);
    _setEnv('XDG_RUNTIME_DIR', '$tmpPath/runtime');
    
    // 12. Change to home directory (like login shell)
    Directory.current = homePath;
    
    // 13. XDG cache home
    final xdgCacheHome = '$prefixPath/tmp/.cache';
    _setEnv('XDG_CACHE_HOME', xdgCacheHome);
    _createDirectory(xdgCacheHome);
    
    // 14. Create XDG runtime directory
    final xdgRuntimeDir = '$tmpPath/runtime';
    _createDirectory(xdgRuntimeDir);
    
    // 15. Print PREFIX for verification (like in your script)
    if (kDebugMode) {
      print('PREFIX: ${_envVars["PREFIX"]}');
    }
  }
  
  /// Create a symlink
  void _createSymlink(String target, String linkPath) {
    try {
      ...
      print('Warning: Failed to create symlink $linkPath -> $target: $e');
    }
  }
  ...
  void _createDirectory(String path) {
    try {
      ...
      print('Warning: Failed to create directory $path: $e');
    }
  }
  ...
  void _setEnv(String key, String value) {
    ...
    // Also set in Platform environment for current process
    Platform.environment[key] = value;
  }
  
  /// Get all environment variables
  Map<String, String> get environmentVars => Map.from(_envVars);
  
  /// Get a specific environment variable
  String? getEnv(String key) => _envVars[key];
  
  /// Generate shell commands to set up environment (for use in scripts)
  String generateShellSetupScript() {
    final buffer = StringBuffer();
    
    buffer.writeln('# Environment setup generated by EnvironmentSetup class');
    buffer.writeln();
    
    // Create symlink
    buffer.writeln('ln -sf $_basePath/containers/$_containerId/tmp $_basePath/usr/tmp');
    
    // Set environment variables
    for (final entry in _envVars.entries) {
      if (entry.key == 'PATH' || entry.key == 'LD_LIBRARY_PATH') {
    ...
    // Create directories
    buffer.writeln('mkdir -p ${_envVars["TMPDIR"]}');
    buffer.writeln('mkdir -p ${_envVars["HOME"]}');
    buffer.writeln('mkdir -p ${_envVars["XDG_CACHE_HOME"]}');
    buffer.writeln('mkdir -p ${_envVars["XDG_RUNTIME_DIR"]}');
    
    // Change to home directory
    buffer.writeln('cd ${_envVars["HOME"]}');
    buffer.writeln('echo ${_envVars["PREFIX"]}');
    
    return buffer.toString();
  }
  
  /// Prepare environment for a process
  Map<String, String> prepareProcessEnvironment({Map<String, String>? additionalVars}) {
    final env = Map<String, String>.from(_envVars);
    
    if (additionalVars != null) {
      env.addAll(additionalVars);
    }
  ...
  bool validateEnvironment() {
    final requiredVars = ['PREFIX', 'HOME', 'TMPDIR', 'PATH', 'LD_LIBRARY_PATH'];
    
    for (final varName in requiredVars) {
      if (!_envVars.containsKey(varName) || _envVars[varName]!.isEmpty) {
    ...
    final requiredDirs = ['HOME', 'TMPDIR'];
    for (final dirVar in requiredDirs) {
      final dirPath = _envVars[dirVar];
      if (dirPath != null && !Directory(dirPath).existsSync()) {
        print('Warning: Directory $dirPath ($dirVar) does not exist');
        return false;
      }
    }
  ...
  /// Reset environment (for testing or reconfiguration)
  void reset() {
    _envVars.clear();
  }
}

// Global environment setup instance
final envSetup = EnvironmentSetup();

// ============================================================================
// DXVK Installer Class (remains the same)
// ============================================================================
...
class Util {
  ...
  static Future<int> execute(String str, {Map<String, String>? environment}) async {
    final env = environment ?? envSetup.prepareProcessEnvironment();
...
class TermPty{
  ...
  TermPty() {
    ...
    // Get prepared environment from EnvironmentSetup
    final processEnv = envSetup.prepareProcessEnvironment();
    
    pty = Pty.start(
      "/system/bin/sh",
      workingDirectory: envSetup.getEnv('HOME') ?? G.dataPath,
...
// ============================================================================
// Workflow Class (Updated with EnvironmentSetup integration)
// ============================================================================
...
class Workflow {
  ...
  static Future<void> setupEnvironment() async {
    ...
    envSetup.initialize(
      basePath: G.dataPath,
      containerId: G.currentContainer.toString(),
    );
    
    // Validate environment
    if (!envSetup.validateEnvironment()) {
      print('Environment validation failed, attempting to fix...');
      // Re-run setup
      envSetup.reset();
      envSetup.initialize(
        basePath: G.dataPath,
    ...
    if (kDebugMode) {
      ...
      envSetup.environmentVars.forEach((key, value) {
        print('  $key=$value');
      });
    }
  }
...
""";
  ...
  static Future<void> setupAudio() async {
    ...
    // Get environment for audio process
    final env = envSetup.prepareProcessEnvironment();
