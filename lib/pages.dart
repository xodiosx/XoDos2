...
/// Reads the current active driver from the opt/drv file or returns "System".
String _getCurrentDriverName() {
  final file = File('/data/data/com.xodos/files/usr/opt/drv');
  if (!file.existsSync()) return 'System';
  final content = file.readAsStringSync();
  // Look for the comment line that marks our custom driver block
  if (content.contains('# Adrenotools custom driver')) {
    // Extract the driver name from ADRENOTOOLS_DRIVER_PATH if possible,
    // but simpler: check which driver folder is referenced.
    final match = RegExp(r'export ADRENOTOOLS_DRIVER_PATH="(.+?)"').firstMatch(content);
    if (match != null) {
      final path = match.group(1)!;
      // The path is something like .../adrenotools/drivers/DriverName/
      final parts = path.split('/');
      if (parts.length >= 2 && parts[parts.length-2] == 'drivers') {
        return parts.last.replaceAll('/', ''); // the driver folder name
      }
    }
...
Future<void> _activateDriver(DriverMeta meta) async {
  await _driverManager.setActiveDriver(meta);
  // Disable all other acceleration options (they are now overridden)
  ...
  // Optionally call your existing apply method to update terminal etc.
  await GpuDriverHelper.applySettingsDirectly();
  setState(() {}); // refresh the panel
}

Future<void> _activateSystemDriver() async {
  await _driverManager.setSystemDriver();
  // No need to change other toggles – they remain as user configured.
  await GpuDriverHelper.applySettingsDirectly();
  setState(() {});
}

Future<void> _installDriver() async {
  // Use file_picker to select a ZIP
  ...
  try {
    ...
    await _driverManager.installDriver(bytes);
    await _refreshDriverList();
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text('Driver installed successfully')),
      );
    }
  } catch (e) {
    if (mounted) {
...
Future<void> _uninstallDriver(DriverMeta meta) async {
  ...
  await _driverManager.uninstallDriver(meta.name);
  // If the active driver was this one, fall back to system
  if (_getCurrentDriverName() == meta.name) {
    await _driverManager.setSystemDriver();
  }
  await _refreshDriverList();
  setState(() {});
...
// Adrenotools manager instance
late AdrenotoolsDriverManager _driverManager;
List<DriverMeta>? _installedDrivers;
...
Future<void> _initDriverManager() async {
  _driverManager = await AdrenotoolsDriverManager.initialize();
  _refreshDriverList();
}

Future<void> _refreshDriverList() async {
  setState(() {
    _installedDrivers = _driverManager.getInstalledDrivers();
  });
}
...
Future<void> _activateDriver(DriverMeta meta) async {
  await _driverManager.setActiveDriver(meta);
  // Turn off all other acceleration options
  await G.prefs.setBool("virgl", false);
  await G.prefs.setBool("venus", false);
  await G.prefs.setBool("turnip", false);
  await G.prefs.setBool("angle", false);
...
Future<void> _activateSystemDriver() async {
  await _driverManager.setSystemDriver();
  // No need to force other toggles off – they work with system driver
  await GpuDriverHelper.applySettingsDirectly();
  setState(() {});
}
